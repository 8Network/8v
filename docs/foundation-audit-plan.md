...
- **Phase 4a** ✓ done. `contract_no_symlink_loops.rs` — 12 active tests, 0 ignored. Every walking command (`ls`, `init` re-init, `search`, `search --files`, `check`, `fmt`, `build`, `test`, `read .`, `read sub/loop`, `stats`, `log`) survives a parent-pointing symlink loop in <5s. Stats/log hang root cause: `aggregator/argv_normalizer.rs` called `canonicalize` per absolute-path token in event argv; on a busy dev box (~220K events in `_8V_HOME`) that meant ~400K syscalls per `8v stats` invocation, and ELOOP-spinning canonicalize on tokens whose parents traversed the loop. Fix: (1) symlink_metadata gate before canonicalize so a symlinked path short-circuits without realpath spin; (2) project + token canonicalize cache so each unique path is resolved once per process. Stats/log tests also set fresh `_8V_HOME` per fixture so the contract is hermetic, not coupled to the dev box's event log size.
...
2. **Stats/log symlink hang** ✓ resolved by Phase 4a-fix: argv_normalizer canonicalize cache + symlink_metadata gate; tests now active and pass in <5s.
3. **`init <outside_path>` policy** ✓ locked as **allow** (current behavior). Phase 4b: 2 active tests in `contract_path_containment.rs` (`init_outside_absolute_path_succeeds`, `init_outside_traversal_path_succeeds`) document that `8v init /tmp/elsewhere` and `8v init ../sibling` succeed by design — init bootstraps a workspace at the supplied path (anchors ContainmentRoot to the arg, not CWD). Tradeoff accepted: legitimate use case (creating a workspace in a chosen location) outweighs the hardening case (typos / hostile prompts escaping into `/etc`).
