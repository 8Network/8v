...
pub fn assemble_agent_context(
    ...
    // ── CLAUDE.md ────────────────────────────────────────────────────────────
    // Order: (1) profile prepend — frames everything below it, (2) common base — shared
    // project rules every profile sees equally, (3) task-specific baseline — kept last
    // so task instructions are never shadowed.
