...
/// Production releases URL — compile-time only, no runtime override.
/// Every released binary is permanently locked to this URL.
/// See design/release.md "Security Constraint: No Runtime URL Override".
const RELEASES_URL: &str = "https://github.com/8network/8v/releases";
...
/// Resolve latest version by following GitHub `/releases/latest` redirect.
///
/// GitHub returns a 302 with `Location: .../releases/tag/vX.Y.Z`. We disable
/// redirect-follow, read the Location header, and parse the tag.
fn fetch_latest_version(base_url: &str) -> Result<String, String> {
    let agent = ureq::builder()
        .timeout_connect(std::time::Duration::from_secs(CONNECT_TIMEOUT_SECS))
        .timeout_read(std::time::Duration::from_secs(DOWNLOAD_TIMEOUT_SECS))
        .redirects(0)
    ...
    let resp = match agent.get(&url).call() {
        Ok(r) => r,
        Err(ureq::Error::Status(301 | 302 | 303 | 307 | 308, r)) => r,
        Err(e) => return Err(format!("could not reach {}: {}", url, e)),
    };

    let location = resp
        .header("location")
        .ok_or_else(|| format!("no location header in response from {}", url))?;

    // Extract last path segment (vX.Y.Z), strip query/fragment and `v` prefix.
    ...
    if version.is_empty() {
        return Err(format!(
            "could not parse version from location: {}",
            location
        ));
    }
...
fn run_impl_report(
    ...
    let remote_version_str = fetch_latest_version(base_url)?;
    ...
    let binary_url = format!(
        "{}/download/v{}/{}",
        base_url, remote_version, binary_filename
    );
    let binary = fetch_binary(&binary_url)?;

    let checksums_url = format!("{}/download/v{}/checksums.txt", base_url, remote_version);
...
impl Command for UpgradeCommand {
    ...
    async fn execute(&self, ctx: &CommandContext) -> Result<Self::Report, CommandError> {
        ...
        // Test affordance — production uses RELEASES_URL; tests override via 8V_RELEASE_BASE_URL env var.
        let base_url = match std::env::var("8V_RELEASE_BASE_URL") {
            Ok(url) => url,
            Err(_) => RELEASES_URL.to_string(),
        };
...
mod tests {
    ...
    fn integration_tampered_binary_rejected() {
        ...
        server.tamper(&format!("download/v0.2.0/{}", filename), b"TAMPERED binary");
    ...
    fn integration_missing_checksums_rejected() {
        ...
        server.remove("download/v0.2.0/checksums.txt");
    ...
    // ─── Test 8: Redirect target injection ─────────────────────────────────

    #[test]
    fn integration_version_injection_rejected() {
        let filename = current_platform_filename();
        let server = o8v_testkit::ReleaseTestServer::start("1.0.0", &[(&filename, b"binary")]);
        // Inject a malicious redirect target — last segment must still parse as semver.
        server.tamper(
            "redirect-target.txt",
            b"/tag/v1.0.0%0A<script>alert(1)</script>",
        );
        ...
        assert!(
            result.is_err(),
            "injected redirect target should fail parse"
        );
    ...
    fn integration_partial_checksum_rejected() {
        ...
        server.tamper("download/v0.2.0/checksums.txt", bad_checksums.as_bytes());
