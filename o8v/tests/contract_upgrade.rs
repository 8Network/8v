...
mod helpers {
    ...
    /// Spawned HTTP/1.1 responder that emits a 302 with `Location: <target>`
    /// for `/latest`, and `body` for any other request (binary/checksums).
    /// Used to mock GitHub Releases redirect-style version discovery.
    pub fn redirect_then_body(target: &str, body: Vec<u8>) -> String {
        ...
        thread::spawn(move || {
            for stream in listener.incoming() {
                ...
                if request_line.contains(" /latest ") {
                    let response = format!(
                        "HTTP/1.1 302 Found\r\nLocation: {target}\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"
                    );
                    let _ = stream.write_all(response.as_bytes());
...
fn upgrade_with_corrupt_checksums_exits_nonzero() {
    ...
    thread::spawn(move || {
        for stream in listener.incoming() {
            ...
            if request_line.contains(" /latest ") {
                let location = format!("/tag/v{newer_clone}");
                let response = format!(
                    "HTTP/1.1 302 Found\r\nLocation: {location}\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"
                );
                let _ = stream.write_all(response.as_bytes());
            ...
            let body: Vec<u8> = if request_line.contains("checksums.txt") {
...
fn upgrade_when_already_current_is_noop() {
    ...
    thread::spawn(move || {
        for stream in listener.incoming() {
            ...
            // /latest → 302 to tag/vX.Y.Z. Anything else = binary/checksum request.
            if request_line.contains(" /latest ") {
                let location = format!("/tag/v{current_ver_clone}");
                let response = format!(
                    "HTTP/1.1 302 Found\r\nLocation: {location}\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"
                );
                let _ = stream.write_all(response.as_bytes());
...
fn upgrade_json_when_already_current() {
    let current_ver = env!("CARGO_PKG_VERSION");
    let base_url =
        helpers::redirect_then_body(&format!("/tag/v{current_ver}"), b"unexpected".to_vec());
