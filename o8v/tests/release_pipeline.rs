...
#[test]
fn binstall_metadata_exists() {
    let root = workspace_root();
    let cargo_path = root.join("o8v/Cargo.toml");
    let content = fs::read_to_string(&cargo_path).expect("o8v/Cargo.toml exists and is readable");

    let parsed: toml::Table = toml::from_str(&content).expect("o8v/Cargo.toml is valid TOML");

    assert!(
        parsed.get("package").is_some(),
        "Cargo.toml has [package] section"
    );
    ...
    assert!(
        package.get("metadata").is_some(),
        "package has [metadata] section"
    );
    ...
    assert!(
        metadata.get("binstall").is_some(),
        "metadata has [binstall] section"
    );
}

#[test]
fn binstall_pkg_fmt_is_bin() {
    let root = workspace_root();
    let cargo_path = root.join("o8v/Cargo.toml");
    let content = fs::read_to_string(&cargo_path).expect("read o8v/Cargo.toml");

    let parsed: toml::Table = toml::from_str(&content).expect("valid TOML");
    ...
    let binstall = parsed
        ...
        .expect("binstall section exists and is a table");

    let pkg_fmt = binstall
        .get("pkg-fmt")
        .and_then(|pf| pf.as_str())
        .expect("pkg-fmt field exists and is a string");

    assert_eq!(
        pkg_fmt, "bin",
        "pkg-fmt must be 'bin' for binstall to work correctly"
    );
}

#[test]
fn binstall_overrides_cover_all_release_platforms() {
    let root = workspace_root();
    let cargo_path = root.join("o8v/Cargo.toml");
    let content = fs::read_to_string(&cargo_path).expect("read o8v/Cargo.toml");
    ...
    let binstall = parsed
        ...
        .and_then(|m| m.get("binstall"))
        .and_then(|b| b.as_table())
        .expect("binstall section exists");
    ...
    for platform in required_platforms.iter() {
        assert!(
            binstall.get("overrides").is_some(),
            "binstall has [overrides] section"
        );
        ...
        assert!(
            overrides.get(*platform).is_some(),
            "binstall.overrides has entry for platform {}",
            platform
        );
    }
}
...
#[test]
fn binstall_urls_use_https_releases_domain() {
    let root = workspace_root();
    let cargo_path = root.join("o8v/Cargo.toml");
    let content = fs::read_to_string(&cargo_path).expect("read o8v/Cargo.toml");
    ...
    let overrides = binstall
        .get("overrides")
        .and_then(|o| o.as_table())
        .expect("overrides table exists");

    for (platform, override_table) in overrides.iter() {
        let override_url = override_table
            .get("pkg-url")
            .and_then(|url| url.as_str())
            .expect("override pkg-url exists for {platform}");

        assert!(
            override_url.starts_with("https://releases.8vast.io/"),
            "override pkg-url for {} must use https://releases.8vast.io/ domain, got: {}",
            platform,
            override_url
        );
    }
...
#[test]
fn binstall_urls_contain_version_placeholder() {
    let root = workspace_root();
    let cargo_path = root.join("o8v/Cargo.toml");
    let content = fs::read_to_string(&cargo_path).expect("read o8v/Cargo.toml");
    ...
    for (platform, override_table) in overrides.iter() {
        ...
        assert!(
            override_url.contains("{version}"),
            "override pkg-url for {} must contain {{version}} placeholder, got: {}",
            platform,
            override_url
        );
    }
